...
add_selectbox = st.sidebar.selectbox(
    ...
    key='llms',
    on_change=lambda: st.session_state.update({"llms": add_selectbox})
)

if add_selectbox=="OPENAI":
    # st.session_state['OPENAIAPI'] = st.sidebar.text_input("API KEY", value=st.session_state['OPENAIAPI'],key='OPENAIAPI')
    st.session_state['OPENAIAPI'] = st.sidebar.text_input("API KEY", value=st.session_state['OPENAIAPI'])
    print("2")
    print(st.session_state)

elif add_selectbox=="AZURE OPEN AI":
    st.sidebar.success("Select a model that you want.")
    
    # The API key for your Azure OpenAI resource.  You can find this in the Azure portal under your Azure OpenAI resource.
    # st.session_state['AZURE_OPENAI_API_KEY'] = st.sidebar.text_input("API KEY", value=st.session_state['AZURE_OPENAI_API_KEY'],key='AZURE_OPENAI_API_KEY')
    ...
    # st.session_state['AZURE_OPENAI_ENDPOINT'] = st.sidebar.text_input("END POINT", value=st.session_state['AZURE_OPENAI_ENDPOINT'],key='AZURE_OPENAI_ENDPOINT')
    st.session_state['AZURE_OPENAI_ENDPOINT'] = st.sidebar.text_input("END POINT", value=st.session_state['AZURE_OPENAI_ENDPOINT'])
    print("2")
    print(st.session_state)

elif add_selectbox=="FAKELLM":
    print("2")
    print(st.session_state)
    pass
